...
// Updating the database to refelct any changes made to the documents decline
 if (isset($_POST['accept'])) {
        # code......
        $get_email = $_SESSION['get_email'];
        $querry = "Select * from babysitter where Email = '$get_email'";
    ...
    // $insert = mysqli_query($conn, "Insert Into request (sitter_name, sitter_other_names,sitter_email,sitter_tel, sitter_add, sitter_tribe,sitter_gender, parent_email, parent_phone, parent_add, parent_tribe, parent_gender,status) 
    //             values ('".$fname."','".$oname."','".$_SESSION['get_email']."','".$phone."','".$address."','".$tribe."','".$gender."','".$_SESSION['gotten_email']."','".$p_phone."','".$p_address."', '".$p_tribe."', '".$p_gender."','accepted')")
    // or die(mysqli_error($con));


    $Check = mysqli_num_rows(mysqli_query($conn, "Select * from requests where Parent_Email = '".$_SESSION['gotten_email']."' and Babysitter_Email = '".$email."'"));

    if ($Check==1) {
      # code...
      $err_msg = "You Have Already Requested For This Babysitter!";
    }else{

      $insert = mysqli_query($conn, "Insert into requests (Parent_Email, Babysitter_Email,Status) values ('".$_SESSION['gotten_email']."','".$email."', 'Pending')");

    if ($insert) {
      # code...
      $suc_msg = "Request Sent Successfully!";
    }else{
     $err_msg = "Sorry Error Encounter!";
    }
}
}
 ...
 include("includes/sidebar.php"); 
  ...
  ?>
              ...
              <div class="panel panel-default">
                ...
                <div class="panel-body" style="border:1px solid  #bcbcbc; padding: 20px;">
                  <form method="POST" action="view_profile.php">
                    ...
                    <label>Gender</label>
                    <select id="gender" name="gender" disabled="" class="form-control" placeholder="Choose Your Gender" required="required" readonly>
                      <option>
                        <?=$Gender?>
                      </option>
                      <option>
      ...
      ?>
          ...
          <div style="margin-left:12px;  margin-bottom:20px; float:right">
              <button class="btn btn-primary btn-md" type="submit" name = "accept" onclick="return confirm('Sure To Send Request?')">  Make Request</button>
               </div> </form>
        </div>
        <!-- /.container-fluid -->
